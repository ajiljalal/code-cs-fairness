...
def fit(net,
    ...
    if net_input is not None:
        ...
        width = 2 # int(img_clean_var.data.shape[2]/totalupsample)
        height = 2 # int(img_clean_var.data.shape[3]/totalupsample)
    ...
    for i in range(num_iter):
        ...
        def closure():
            ...
            # the actual loss
            true_loss = torch.zeros_like(loss) # mse(Variable(out.data, requires_grad=False), img_clean_var)
            mse_wrt_truth[i] = true_loss.data.cpu().numpy()
            if i % 10 == 0:
                out2 = net(Variable(net_input_saved).type(dtype))
                loss2 = torch.zeros_like(loss) # mse(out2, img_clean_var)
