...
class LossBuilder(torch.nn.Module):
    def __init__(self, ref_im, loss_str, eps, hr_size):
        super(LossBuilder, self).__init__()
        assert ref_im.shape[2]==ref_im.shape[3]
        im_size = ref_im.shape[2]
        factor=hr_size//im_size
        assert im_size*factor==hr_size
        self.D = torch.nn.AvgPool2d(factor) #BicubicDownSample(factor=factor)
        self.ref_im = ref_im
