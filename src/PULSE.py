...
class PULSE(torch.nn.Module):
    def __init__(self, image_size, checkpoint_path, dataset, verbose=True):
        ...
        self.model = Generator(image_size, 512,8)
        self.model.load_state_dict(torch.load(checkpoint_path)["g_ema"], strict=False)
        self.model.eval()
        self.model.cuda()
        ...
        if Path(f"{dataset}_gaussian_fit.pt").exists():
            self.gaussian_fit = torch.load(f"{dataset}_gaussian_fit.pt")
        else:
            with torch.no_grad():
    ...
    def forward(self, ref_im,
        ...
        noises_single = self.model.make_noise()
        noises = []
        noise_vars = []
        ...
        opt_func = opt_dict[optimizer_type]
        opt = SphericalOptimizer(opt_func, var_list, lr=learning_rate)

        schedule_dict = {
            'fixed': lambda x: 1,
            'linear1cycle': lambda x: (9*(1-np.abs(x/max_update_iter-1/2)*2)+1)/10,
            'linear1cycledrop': lambda x: (9*(1-np.abs(x/(0.9*max_update_iter)-1/2)*2)+1)/10 if x < 0.9*max_update_iter else 1/10 + (x-0.9*max_update_iter)/(0.1*max_update_iter)*(1/1000-1/10),
        }
        schedule_func = schedule_dict[lr_schedule]
        scheduler = torch.optim.lr_scheduler.LambdaLR(opt.opt, schedule_func)
        
        loss_builder = LossBuilder(ref_im, loss_str, pulse_eps, self.image_size).cuda()
        ...
        for j in range(max_update_iter):
            opt.opt.zero_grad()
            ...
            gen_im = self.model([latent_in],input_is_latent=True, noise=noises)[0] * 0.5 + 0.5
